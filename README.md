...
## Запуск сервера
При запуске скрипт будет по умолчанию использовать настройки из файла `backend\config.json`(путь до файла можно изменить в системной переменной 'CFG-PATH'). Данный файл имеет следующую структуру:
```
{
